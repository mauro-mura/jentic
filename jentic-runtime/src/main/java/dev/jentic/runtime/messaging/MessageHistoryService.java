...
/**
 * Thread-safe service for storing and querying message history.
 *
 ...
 *
 * <p>Can be attached to a {@link MessageService} as a subscriber to
 * automatically capture all messages flowing through the bus.
 *
 * <p><strong>Usage as subscriber:</strong>
 * <pre>{@code
 * MessageHistoryService history = new MessageHistoryService(1000);
 * history.attachTo(messageService);
 * 
 * // Later: query history
 * List<StoredMessage> recent = history.getRecent(10);
 * }</pre>
 *
 * @since 0.4.0
 * @see MessageService
...
public class MessageHistoryService {
    ...
    /**
     * Attaches this history service to a MessageService as a subscriber.
     *
     * <p>All messages sent through the MessageService will be automatically
     * stored in this history service.
     *
     * @param messageService the message service to attach to
     * @return this instance for method chaining
     * @throws IllegalStateException if already attached
     */
    public MessageHistoryService attachTo(MessageService messageService) {
        Objects.requireNonNull(messageService, "messageService cannot be null");
        
        if (subscriptionId != null) {
            throw new IllegalStateException("Already attached to a MessageService");
        }

        this.subscriptionId = messageService.subscribe(
                msg -> true,  // Accept all messages
                this::handleMessage
        );

        log.info("MessageHistoryService attached to MessageService (subscription: {})", subscriptionId);
        return this;
    }

    /**
     * Detaches from the MessageService.
     *
     * @param messageService the message service to detach from
     */
    public void detachFrom(MessageService messageService) {
        if (subscriptionId != null && messageService != null) {
            messageService.unsubscribe(subscriptionId);
            log.info("MessageHistoryService detached (subscription: {})", subscriptionId);
            subscriptionId = null;
        }
    }

    /**
     * Checks if this service is attached to a MessageService.
     */
    ...
    public void store(Message message) {
        ...
        log.trace("Stored message: id={}, topic={}, size={}", message.id(), message.topic(), size);
    }
