...
class SummarizationStrategyTest {
    ...
    void selectMessages_shouldRespectTokenBudget() {
        ...
        int messageTokens = result.stream()
            .mapToInt(msg -> estimator.estimateTokens(msg))
            .sum();

        // Replicate the placeholder the strategy builds internally:
        // 30 messages total, 10 recent kept → 20 summarized
        int oldMessageCount = 30 - 10;
        String placeholder = String.format(
            "[Summary of %d earlier messages would be generated by LLM here]", oldMessageCount);
        int actualSummaryTokens = estimator.estimateTokens(placeholder);

        assertThat(messageTokens + actualSummaryTokens).isLessThanOrEqualTo(budget);
    }
