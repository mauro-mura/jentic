...
public class ScheduledExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Thread.sleep(15000); // Let tasks run for 15 seconds
    ...
    /**
     * Demonstrate basic cron scheduling patterns.
     */
    private static void demonstrateBasicScheduling(JenticRuntime runtime) {
        log.info("Creating various scheduled behaviors...\n");
        
        // 1. Every second (for demo purposes)
        ScheduledBehavior everySecond = new ScheduledBehavior(
            "every-second",
        ...
        everySecond.onSuccess(b -> 
            log.debug("   ✓ Completed in {}ms", b.getAverageExecutionTimeMs())
        );
        
        // 2. Every 5 seconds
        ScheduledBehavior everyFiveSeconds = new ScheduledBehavior(
            "every-five-seconds",
            "*/5 * * * * *"  // Every 5 seconds
        ) {
            @Override
        ...
        // 3. Using factory method - every hour (won't execute in this short demo)
    ...
    /**
     * Demonstrate real-world scheduling use cases.
     */
    private static void demonstrateRealWorldUseCases(JenticRuntime runtime) {
        
        // 1. Business Hours Task (9 AM - 5 PM, Mon-Fri)
        ...
        // 3. Cleanup Task (Every Sunday at midnight)
        ScheduledBehavior weeklyCleanup = new ScheduledBehavior(
            "weekly-cleanup",
            "0 0 0 * * SUN"  // Midnight on Sunday
        ) {
            @Override
            protected void scheduledAction() {
        ...
        // 4. Monitoring Task with error handling
        ScheduledBehavior monitoring = new ScheduledBehavior(
            "system-monitor",
            "*/10 * * * * *"  // Every 10 seconds
        ) {
        ...
        monitoring.onFailure(e -> 
            log.warn("   ⚠️  Monitoring failed: {}", e.getMessage())
        );
    ...
    private static void demonstrateMetrics(JenticRuntime runtime) throws Exception {
        ...
        ScheduledBehavior metricsDemo = new ScheduledBehavior(
            "metrics-demo",
            "*/2 * * * * *"  // Every 2 seconds
        ) {
            ...
            protected void scheduledAction() throws Exception {
                ...
                if (taskNumber % 5 == 0) {
                    throw new RuntimeException("Simulated failure for metrics");
                }
            }
        };
        
        metricsDemo.onSuccess(b -> {
            if (b.getTotalExecutions() % 3 == 0) {
                log.info("   📊 Metrics Update: {}", b.getMetricsSummary());
            }
        });
        
        metricsDemo.onFailure(e -> 
            log.error("   ❌ Task failed: {}", e.getMessage())
        );
        
        registerBehaviorWithAgent(runtime, metricsDemo);
        
        // Let it run and show periodic metrics
    ...
    /**
     * Helper method to register a behavior with the first available agent.
     */
...
/**
 * Demo agent for scheduled behaviors.
 */
@JenticAgent(
    value = "scheduler-demo",
    type = "scheduler",
    capabilities = {"scheduled-tasks", "cron", "automation"}
)
class SchedulerDemoAgent extends BaseAgent {
    
    private static final Logger log = LoggerFactory.getLogger(SchedulerDemoAgent.class);
    
    public SchedulerDemoAgent() {
        super("scheduler-demo", "Scheduler Demo Agent");
    }
